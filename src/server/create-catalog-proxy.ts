...
export const SERVER_NAME = 'agentage-catalog-mcp';
export const SERVER_TITLE = 'Agentage MCP Catalog';

const INSTRUCTIONS =
  'A public directory of Model Context Protocol (MCP) servers, crawled from the official registry. Use it to discover, compare, and look up MCP servers on request ("is there an MCP for X", "which MCP server does Y"). catalog__search finds servers by keyword and filters; catalog__get returns one server\'s full packages, tools, and install command by slug; catalog__facets lists the category/language/license values you can filter on. Read-only - it never installs or runs anything. This package is a thin stdio proxy to catalog.agentage.io/mcp; the tool set is served by the remote and may evolve.';

// A low-level MCP Server that forwards tools/list and tools/call to the remote
// catalog verbatim - tools are NOT hard-coded, so upstream changes flow through
// without republishing this package.
export const createCatalogProxyServer = (upstream: UpstreamClient): Server => {
